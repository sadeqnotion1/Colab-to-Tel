...
if setup_ok:
    ...
    log.info(f"Current Directory (before cd): {os.getcwd()}")
    ...
    log.info(f"Listing contents of {colab_dir_path}:")
    ipython.system(f'ls -l "{colab_dir_path}"')
    log.info(f"First 15 lines of {main_script_path}:")
    ...
    try:
        ...
        # FIXED: Use subprocess to capture ALL output including errors
        log.info(f"Executing: python3 -m {bot_main_module}")
        print("\n" + "="*80)
        print("BOT OUTPUT (stdout + stderr):")
        print("="*80 + "\n")
        ...
        result = subprocess.run(
            ...
            cwd=repo_path,
            capture_output=False,  # Don't capture - show directly
            text=True
        )
