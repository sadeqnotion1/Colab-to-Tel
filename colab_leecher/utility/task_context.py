...
class SystemMetrics:
    ...
    def get_avg_completion_time(self) -> float:
        ...
        if not self.recent_completion_times:
            return 0.0
        return sum(self.recent_completion_times) / \
            len(self.recent_completion_times)
...
class TaskTransfer:
    ...
    # List of {'url': url, 'filename': filename}
...
class TaskMessages:
    ...
    # Current file being processed (archiving/extracting)
    current_file: str = ""
...
class TaskContext:
    ...
    # Timestamp of last status bar update (for throttling)
...
class TaskQueue:
    ...
    async def remove_task(self, task_id: str) -> Optional[TaskContext]:
        ...
        async with self._lock:
            ...
            if task_ctx:
                ...
                # Track data volume
                self.metrics.total_bytes_downloaded += sum(
                    task_ctx.transfer.down_bytes) if isinstance(
                    task_ctx.transfer.down_bytes,
                    list) else task_ctx.transfer.down_bytes
                self.metrics.total_bytes_uploaded += sum(
                    task_ctx.transfer.up_bytes) if isinstance(
    ...
    async def shutdown(self):
        ...
        try:
            ...
            log.warning(
                "Shutdown timeout - some tasks may have been force-killed")
            ...
            for task_id, task_ctx in self.active_tasks.items():
                if task_ctx.is_completed or task_ctx.is_cancelled:
                    if task_ctx.completed_at:
                        age_hours = (
                            now - task_ctx.completed_at).total_seconds() / 3600
                        if age_hours > max_age_hours:
                            to_remove.append(task_id)

            # Remove directly from dict to avoid nested lock acquisition
            # (deadlock)
